...
def prediction_page(conn, cursor):
    ...
    if forecast_months > 12:
        st.warning(f"You chose to predict for {forecast_months} months, this is {forecast_months - 12} month(s) more than standard 12 months forecast.")
                ...
                if y_pred is not None:
                    ...
                    try:
                        ...
                        if model_id is not None:
                            ...
                            logging.error("model_id not found.")
                    except sqlite3.Error as e:
                        st.error(f"Failed to add prediction or evaluation to database: {e}")
                        logging.exception(f"Database error: {e}")
                else:
                    st.error("Transformer prediction failed.")
                    logging.error("Transformer prediction failed.")
    else:

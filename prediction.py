...
import numpy as np
import sqlite3
from utils import load_dataset, scale_data, check_stationarity, split_data_arima, evaluate_predictions, plot_predictions, generate_recommendation, compare_actual_vs_predicted, transformer_prediction_final, load_transformer_model_final, evaluate_arima_model, load_arima_model  # Import load_arima_model dari utils
...
def prediction_page(conn, cursor):
    ...
    if st.button("Preprocess and Run Prediction & Evaluation"):
        if algorithm == "ARIMA":
            model_dict = load_arima_model() # remove this

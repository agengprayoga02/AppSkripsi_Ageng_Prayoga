    ...
    try:
        if os.path.exists(model_path):
            with open(model_path, "rb") as f:
                model = pickle.load(f)
            if isinstance(model, SARIMAX):  # Memastikan model adalah tipe yang benar
                logging.info(f"ARIMA model loaded from: {model_path}")
                return model
            else:
                st.error(f"Object loaded from {model_path} is not a SARIMAX model.")
                logging.error(f"Object loaded from {model_path} is not a SARIMAX model.")
                return None
        else:
...
def evaluate_arima_model(df, forecast_months):
    ...
    try:
        ...
        if scaler is None:
            ...
            return None, None
        logging.info("ARIMA scaler loaded successfully.")
        ...
        scaled_data, scaler_obj = scale_data(df)  # Mengambil scaler object
        ...
        if scaled_data is not None:
            ...
            try:
                ...
                y_pred_original = scaler_obj.inverse_transform(np.array(y_pred).reshape(-1, 1)).flatten()  # Menggunakan scaler object yang diperoleh dari scale_data
